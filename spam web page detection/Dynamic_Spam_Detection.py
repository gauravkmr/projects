...
#-------------------------------------------------
## Crawler method to fetch all htm and html pages
#-------------------------------------------------
def crawl( seeds ):
                
    depth_till_traverse = 10     # Do a BFS and decide the level for the traversal through this variable
    ...
    for crawl_url in frontier:
        if len( visited_url ) == depth_till_traverse:
            break
...
def main():
    ...
    f = open('C:/Python27/html/stopword.txt')
    for line in f :
    ...
    crawl(['http://www.yahoo.com/'])        # Crawl the yahoo directory to dynamically fetch pages and classify them as spam or non-spam
    br = mechanize.Browser()    
    br.set_proxies({})

    # Now dynamically fetch each of the crawled URLs
    for url in discovered_url:
        try:
            br.open(url)
        except Exception as e:
            #print e
        ...
        #-------------------------------------------------------------------------
        # Reading dynamically fetched page And Removing HTML Tags and punctuations
        #-------------------------------------------------------------------------
        
        parenthtml = br.response().read()
        a = nltk.clean_html( parenthtml )   
        a = ' '.join( a.split() )
